...
$id = $_POST['id'];
$query = "SELECT * FROM marca_vehiculo  WHERE id_marca=:id";
$statement = $conexion->getConnection()->prepare($query);
$statement->bindParam(':id', $id);
$statement->execute();
$marca = $statement->fetch(PDO::FETCH_ASSOC);
?>
...
<a class="navbar-brand">
  ...
  <div class="form-container">
    <form action="../Update/Update_marca.php" method="POST">

      <input type="text" name="opcion" value="4" hidden>

      <input type="text" name="id"  value="<?php echo $marca['id_marca'];?>" hidden >
      <div class="input-group">
        <label>Nombre de la marca:</label>
        <input type="text" name="nombremarca"  required  value=" <?php echo $marca['marca'];?>">
      </div>
